...
export default function CartPage(){
  ...
  return (
    <div className='container py-6' data-testid='cart-page'>
      <div className='md:hidden mb-3'>
        <button
          ...
          data-testid='cart-back-to-catalog'
        >
          В каталог
        </button>
      </div>
      ...
      {cart.length===0 ? <p className='text-gray-600' data-testid='cart-empty'>Кошик порожній</p> : (
        <div className='space-y-2 w-full lg:w-4/5 mx-auto' data-testid='cart-content'>
            <div className={`${cart.length>7 ? 'max-h-[60vh] md:max-h-[520px] overflow-y-auto pr-1 space-y-2' : 'space-y-2'}`} data-testid='cart-items'>
              {cart.map(i=> (
                <div key={i._id} className='p-3 bg-white rounded shadow flex flex-col sm:flex-row sm:flex-wrap md:flex-nowrap sm:items-center justify-between gap-3' data-testid={`cart-item-${i._id}`}>
                  <div className='flex items-center gap-3 min-w-0 flex-1 cursor-pointer pr-1 sm:pr-3 md:pr-4' onClick={()=> setSelected(i)} data-testid={`cart-item-${i._id}-open`}>
                    ...
                    <div className='flex flex-col min-w-0'>
                      <div className='font-medium truncate' data-testid={`cart-item-${i._id}-name`}>{i.name}</div>
                      {i.sku && (
                  ...
                  <div className='flex items-center gap-3 sm:gap-5 shrink-0 sm:mt-0 w-full sm:w-auto sm:min-w-[260px] justify-between sm:justify-end'>
                    <div className='font-medium whitespace-nowrap text-right text-sm md:text-base' data-testid={`cart-item-${i._id}-sum`}>{(i.price * i.quantity).toFixed(2)} ₴</div>
                    <div className='flex items-center gap-1.5 md:gap-2'>
                      <button onClick={()=>dec(i._id)} className='w-7 h-7 md:w-8 md:h-8 border rounded flex items-center justify-center text-sm transition active:scale-95' data-testid={`cart-item-${i._id}-dec`}>−</button>
                      <span className='w-8 md:w-10 text-center text-sm md:text-base' data-testid={`cart-item-${i._id}-qty`}>{i.quantity}</span>
                      <button onClick={()=>inc(i._id)} className='w-7 h-7 md:w-8 md:h-8 border rounded flex items-center justify-center text-sm transition active:scale-95' data-testid={`cart-item-${i._id}-inc`}>+</button>
                    </div>
                    <button onClick={()=>remove(i._id)} className='text-red-600 transition active:scale-95 text-base md:text-lg' title='Видалити' aria-label='Видалити' data-testid={`cart-item-${i._id}-remove`}>✖</button>
                  </div>
                </div>
              ))}
            </div>
            <div className='mt-2 md:mt-3 p-3 md:p-4 bg-gray-50 border border-gray-200 rounded-lg shadow-md flex flex-row items-center justify-between gap-2 md:gap-3'>
              ...
              <div className='flex gap-2 w-auto items-center justify-end md:justify-center'>
                ...
                <a href='/checkout' className='hidden md:inline-flex btn btn--sm transition active:scale-95 justify-center' data-testid='cart-checkout'>Замовити</a>
                <button onClick={clear} className='px-2 py-2 md:px-3 md:py-2 border rounded transition active:scale-95 w-10 h-10 md:w-auto md:h-auto flex items-center justify-center text-sm md:text-base' title='Очистити кошик' aria-label='Очистити кошик' data-testid='cart-clear'>
      ...
      {selected && (()=>{
        ...
        return (
          <div className='fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4' onClick={()=> setSelected(null)} data-testid='cart-item-modal-overlay'>
            <div className='relative bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-hidden transition' onClick={e=>e.stopPropagation()} data-testid='cart-item-modal'>
              <button
                aria-label='Закрити'
                ...
                data-testid='cart-item-modal-close'
              >
                <FiX className='w-5 h-5' />
              </button>
              <div className='p-4 md:p-6'>
                <div className='bg-gray-50 border rounded-xl shadow-sm p-2'>
                  <img src={resolveSrc(live.image) || NO_IMG_LARGE} className='mx-auto w-full md:w-4/5 max-h-[32vh] md:max-h-[38vh] object-contain rounded-md' alt={live.name} data-testid='cart-item-modal-image' />
                </div>
                <h3 className='mt-4 text-2xl font-semibold tracking-tight text-center' data-testid='cart-item-modal-title'>{live.name}</h3>
                {live.sku && (
                  <div className='mt-1 text-sm text-gray-500 text-center'>Артикул: {live.sku}</div>
                )}
                <div className='mt-3 text-gray-700 whitespace-pre-wrap max-h-[28vh] overflow-auto pr-1 text-center' data-testid='cart-item-modal-description'>
